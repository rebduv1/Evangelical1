...
function Nav({ page, setPage }) {
  ...
  const navItems = [
    ...
    { key: "survey", label: "Take Our Survey" },
    { key: "faq", label: "FAQ" },
  ...
  return (
    <section style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden", background: COLORS.deepBlue }}>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "120px 32px 80px", textAlign: "center", position: "relative", opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(30px)", transition: "all 1.2s cubic-bezier(0.22, 1, 0.36, 1)" }}>
        ...
        <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(24px, 3.5vw, 34px)", fontStyle: "italic", color: "rgba(232,213,160,0.9)", lineHeight: 1.5, marginBottom: 12, fontWeight: 700 }}>
          "I will bless those who bless you" <span style={{ fontStyle: "normal", fontSize: "0.75em", opacity: 0.7 }}>— Genesis 12:3</span>
        </p>
...
function HomePage({ setPage }) {
  // Auto-open Tally survey after 8 seconds — only once per visit
  useEffect(() => {
    if (window._tallyShown) return;
    const timer = setTimeout(() => {
      if (window.Tally) {
        window.Tally.openPopup("yPjWWW", {
          layout: "modal",
          width: 700,
          emoji: { text: "👋", animation: "wave" },
          showOnce: true,
        });
        window._tallyShown = true;
      }
    }, 8000);
    ...
    <div>
      ...
      <section style={{ background: COLORS.cream, padding: "80px 24px" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
            ...
            <InfoCard icon="🤝" title="Beyond Advocacy">Christian organizations that support Israel and the Jewish people have mobilized millions for political support. This adds a deeply personal dimension: directly supporting the religious education that sustains Jewish identity.</InfoCard>
      ...
      <section style={{ background: COLORS.cream, padding: "80px 24px" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          <div style={{ width: 64, height: 64, borderRadius: "50%", background: COLORS.softGold, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 32, margin: "0 auto 24px" }}>📋</div>
          <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(28px, 4vw, 40px)", fontWeight: 700, color: COLORS.navy, lineHeight: 1.2, marginBottom: 16 }}>Help Shape This Initiative</h2>
          <p style={{ fontFamily: "'EB Garamond', serif", fontSize: 18, color: COLORS.slate, lineHeight: 1.7, marginBottom: 32, maxWidth: 580, margin: "0 auto 32px" }}>
            Your perspective matters. Take a brief survey to help us understand how the Evangelical community can most effectively partner with Jewish families through the Education Freedom Tax Credit. Your honest input — including critical input — is what we most need.
          </p>
          <button
            onClick={() => { setPage("survey"); window.scrollTo({ top: 0, behavior: "smooth" }); }}
            style={{
              background: `linear-gradient(135deg, ${COLORS.gold}, #b8943f)`, color: COLORS.deepBlue,
              border: "none", padding: "18px 48px", borderRadius: 8,
              fontFamily: "'EB Garamond', serif", fontSize: 18, fontWeight: 700, cursor: "pointer",
              boxShadow: "0 4px 24px rgba(201,168,76,0.3)", letterSpacing: 0.5,
            }}
          >
            Take Our Survey →
          </button>
...
function SurveyPage() {
  return (
    <div style={{ paddingTop: 80 }}>
      <section style={{
        background: `linear-gradient(170deg, ${COLORS.deepBlue}, ${COLORS.navy})`,
        padding: "60px 24px 30px", textAlign: "center", position: "relative", overflow: "hidden",
      }}>
        <div style={{ position: "absolute", inset: 0, opacity: 0.03,
          backgroundImage: `repeating-linear-gradient(45deg, ${COLORS.gold} 0, ${COLORS.gold} 1px, transparent 1px, transparent 20px)`,
        }} />
        <div style={{ position: "relative", maxWidth: 640, margin: "0 auto" }}>
          <p style={{ fontFamily: "'EB Garamond', serif", fontSize: 13, letterSpacing: 3, color: COLORS.gold, textTransform: "uppercase", marginBottom: 12 }}>Your Voice Matters</p>
          <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(30px, 5vw, 44px)", fontWeight: 700, color: COLORS.cream, lineHeight: 1.2, marginBottom: 16 }}>
            Take Our <span style={{ color: COLORS.gold }}>Survey</span>
          </h1>
          <p style={{ fontFamily: "'EB Garamond', serif", fontSize: 17, color: "rgba(232,213,160,0.75)", lineHeight: 1.6 }}>
            Help us understand how the Evangelical community can most effectively partner with Jewish families through the Education Freedom Tax Credit.
          </p>
        </div>
      </section>
      <section style={{ background: COLORS.cream, padding: "0 24px 40px" }}>
        <div style={{ maxWidth: 800, margin: "0 auto" }}>
          <iframe
            data-tally-src="https://tally.so/r/yPjWWW?transparentBackground=1"
            src="https://tally.so/r/yPjWWW?transparentBackground=1"
            width="100%"
            height="800"
            frameBorder="0"
            marginHeight="0"
            marginWidth="0"
            title="Christian Perspectives on Supporting Torah-true Jewish Education"
            style={{ border: "none", minHeight: 800, borderRadius: 8 }}
          />
        </div>
      </section>
    </div>
        ...
        <div style={{ maxWidth: 620, margin: "0 auto", background: COLORS.warmWhite, borderRadius: 16, border: `1px solid rgba(201,168,76,0.15)`, padding: "40px 36px", boxShadow: "0 4px 24px rgba(0,0,0,0.04)" }}>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              {["$170", "$340", "$1,700 (Maximum)", "Other"].map((amt) => (
